...
<div id="container">
 <?php
    ...
    if (empty($_POST['restName']) && empty($_POST['restTel']) && empty($_POST['restDes'])) {
		echo "<p>Please Fill in all information!</p>";
        echo '<meta http-equiv=REFRESH CONTENT=5;url=memberpanel.php>';
	} else {
        $restName = $_POST['restName'];
		$restTel = $_POST['restTel'];
		$restDes = $_POST['restDes'];
	
	    $id = $_SESSION['username'];
		$sql = "update restaurants set rname = '".$restName."', rtel = '".$restTel."', rdes = '".$restDes."' where owner = '".$id."'";
		if (mysql_query($sql)) {
        echo "Infomation Changed!</br>";
        echo "<a href='memberpanel.php'>back</a>";
	} else 	{
        echo "Failed to change information</br>";
        echo "<a href='memberpanel.php'>back</a>";
    }
    }
 ?>
