...
<?php
    ...
    if (isset($_SESSION['username'])) {
		...
		if ($pass != null && $repass != null && $tel != null) {
		    $temp = "select Password from acc where UserName='".$id."'";
		    $result = mysql_query($temp);
		    $row = mysql_fetch_row($result);
			if ($pass == $repass && $row[0] == $pass && strlen($tel) == 8) {
			    $sql = "update acc set Tel = '".$tel."' where UserName = '".$id."'";
				if (mysql_query($sql)) {
				    echo "<p style='color:#0000FF; padding:25px 0 25px 15px; text-align:center'>Information Updated! Redirecting....</p>";
                    echo "<meta http-equiv=REFRESH CONTENT=2;url=memberpanel.php>";
				} else {
                    echo "<p style='color:#0000FF; padding:25px 0 25px 15px; text-align:center'>Failed to update! Redirecting....</p>";
                    echo "<meta http-equiv=REFRESH CONTENT=2;url=changeinfo.php>";	
                }			
            } else { 	
                echo "<p style='color:#0000FF; padding:25px 0 25px 15px; text-align:center'>Failed to update! Redirecting....</p>";
                echo "<meta http-equiv=REFRESH CONTENT=2;url=changeinfo.php>";		
